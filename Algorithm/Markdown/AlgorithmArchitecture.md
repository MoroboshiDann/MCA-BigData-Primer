...
- 对于列上的限制，可以直接通过与操作，将当前皇后的站位与col进行与操作，即可将当前皇后的列限制加入。
- 对于左下角和右下角的限制，除了考虑当前行的影响，还要考虑之前行的影响，否则可能会出现与之前皇后同对角线的情况。
    - 于是就要先将之前的leftCorner左移一位，然后与上当前皇后造成的列限制。
...
一般来说，滑动窗口的左右边界都只会往一个方向移动，不需要回溯。左边界left向右移动，就会有元素从窗口中移动出去；右边界right向右移动，就会有元素加入到窗口中。



## 窗口最值

滑动窗口问题中，经常考察窗口内的最值问题。如果每次窗口发生变化，都通过遍历的方式获取最值，时间复杂度就会比较高。

### 双端队列

可以通过双端队列来优化时间复杂度。

双端队列内部元素要求从尾部(入口)到头部(出口)非递减。即头部元素是最大值。实现过程如下：

- 当窗口扩大时，有新元素加入窗口。
    - 检查尾部元素是否比新元素大，如果是，则直接将新元素加入到双端队列尾部。
    - 如果尾部元素比新元素小，将尾部元素弹出，直到尾部元素比新元素大。
- 当窗口缩小时，有元素退出窗口。
    - 检查退出元素是否是队列头部元素，若果是就将头部元素出队，否则不用处理。
...
public int[] maxSlidingWindow(int[] arr, int k) {
    ...
    for (int i = 0; i < k; ++i) {
        while (!queue.isEmpty() && queue.peekLast() < arr[i]) queue.pollLast();
        queue.offer(arr[i]);
    }
    int length = arr.length;
    ...
    for (int i = k; i < length; ++i) {
        if (queue.peek() == arr[i - k]) queue.poll();
        while (!queue.isEmpty() && queue.peekLast() < arr[i]) queue.pollLast();
        queue.offer(arr[i]);
        ans[index++] = queue.peek();
    }
...
可以使用对结构来维护窗口中的最大值。窗口缩小时，只需要判断移出的元素是否是堆顶元素即可。
...
public int[] maxSlidingWindow(int[] arr, int k) {
    ...
    ans[index++] = heap.peek();
    for (int i = k; i < length; ++i) {
        if (heap.peek() == arr[i - k]) heap.poll();
        heap.offer(arr[i]);
        ans[index++] = heap.peek();
    }
    return ans;
...
首先，对于滑动窗口中的最大值和最小值，分别使用一个双端队列来维护。
...
给定一个二维数组matrix，其中的元素由0和1组成。返回全部由1组成的矩形中，包含1的个数最多的数量。

### 暴力解

n* n的矩阵，内部包含n^4个子矩阵。直接遍历所有的子矩阵，统计最大的子矩阵内部的1的数量即可。

### 单调栈

遍历数组每一行，对于当前行来说，计算以每一个位置作为“地基”的子矩阵有多高。即计算从当前位置往上开始有几个连续的1，如果当前位置为0，高度直接为0。

则可以利用一个一维数组height，初始设置置为二维数组第0行。然后开始遍历，如果二维数组当前行的当前位置元素为0，就直接将height对应位置置为0；如果是1，就与height对应位置相加。

每一行得到的height数组，就等同于直方图问题中的数组。从而将问题转化为，求直方图中面积最大的矩形。


```java
public int maxAmountOfOne(int[][] matrix) {
    int[] height = new int[matrix[0].length];
    int ans = Integer.MIN_VALUE;
    for (int i = 0; i < matrix.length; ++i) {
        for (int j = 0; j < height.length; ++j) {
            height[j] = matrix[i][j] == 0 ? 0 : height + matrix[i][j];
        }
...
给定一个二维数组matrix，其元素由0和1组成。求全部由1组成的子矩阵的数量。
...
同样使用压缩矩阵的做法，求以每一行为“地基”的，全部由1构成的子矩阵的数量。然后再计算所有行的求和。此时，就将问题分解了。

对于每一行，求以每一个位置为“高”的内部全部为1的子矩阵的数量，然后求和，就可以得到当前行的目标数量。
...
给定一个字符串，求出字符串中最长的回文子串的长度。字串必须是连续的。



### 暴力解法

遍历字符串，以每一个位置为中心点向两边扩展，计算回文串的长度。

如果，回文子串的长度是偶数，上述方法就不可行。所以，将字符串进行处理，给字符串的开头和结尾添加一个特殊字符，如`#`，并给每两个字符中间插入一个特殊字符。这样就不用考虑就问题了。以特殊字符为中心扩展时，求的就是偶数长度的回文字符串。

注意，特殊字符可以是任意字符，即便是已经出现在字符串中的字符，不会影响结果。
