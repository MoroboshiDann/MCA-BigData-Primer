...
​		对于数组`nums[n]`，定义它的前缀数组`presum[n]`，有`presum[i] = sum(nums[0...i])`。通过前缀和数组，可以轻松输出nums的某个区间内的数，如`sum(nusm[i...j]) = presum[j] - presum[i - 1]`。

> 通常为了计算方便，会申请`int[] presum = new int[n + 1]`。

### 扩展到二维

​		对于数组`nums[m][n]`，定义它的前缀和`presum[m][n]`，表示以该坐标为右下角的矩形内的元素求和。则可以通过以下图形进行求和。同时为了方便，行数和列数都会比原数组加一。

![](../img/presum.png)

```java
void init(int[][] nums) {
    int[][] presum = new int[nums.length + 1][nums[0].length];
...
​		当要求图形中任意矩形区域元素的求和时，同样按照矩形面积求和。

![](../img/presum2.png)

```java
int squareSum(int r1, int c1, int r2, int c2) {
...
​		同前缀和数组一样，差分数组也是为了快速求解某些问题出现的。定义`diff[n] = nums[n] - nums[n - 1]; diff[0] = nums[0];`。

```java
int[] diff = new int[batteryPercentages.length];
diff[0] = batteryPercentages[0];
int temp = diff[0];
for (int i = 1; i < diff.length; ++i) {
    diff[i] = batteryPercentages[i] - temp;
    temp = batteryPercentages[i];
}
```
...
​		差分数组常用来处理给数组某个区间内的元素加上或减去i。如给nums的[j, k]的元素加上3，只需要给差分数组的diff[j]加上3，然后给diff[k + 1]减去3，即可给原数组该区间内元素加3。原理就是给差分数组某个位置加3，就是给该元素往后所有元素加3。
...
​		快慢指针相遇，证明链表有环，此时设，链表头到环的入口的长度为a，入口到相遇点的距离为b，相遇点再次到入口的距离为c，根据快慢指针给做的路程有：

- a + n(b + c) + b：这是快指针走过的路程，两者能够相遇，那在相遇点之前，快指针一定已经在环中走了n圈，然后又走了b，与慢指针相遇
- a + b：满指针的路程
...
最终推出的`a = c + (n -1)(b + c)`表名，从链表头到入口的a，等于先走到入口，走了c，然后再走n个环长。